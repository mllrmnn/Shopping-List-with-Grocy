...
async def async_suggest_grocery_list_service(call):
    ...
    product_entities = [
        ...
        and entry.platform == "shopping_list_with_grocy_polling"
...
def async_setup_services(hass) -> None:
    ...
    async def async_call_shopping_list_with_grocy_polling_service(service_call) -> None:
    ...
    hass.services.async_register(
        DOMAIN,
        SERVICE_REFRESH,
        async_call_shopping_list_with_grocy_polling_service,
        ...
        try:
            ...
            if result["success"]:
                ...
                for match in result.get("matches", []):  # Show all matches
                    matches_info.append(
                        f"â€¢ {match.get('name', 'Unknown')} (ID: {match.get('id', 'Unknown')})"
                    )
    ...
    async def async_select_choice_by_number_service(service_call) -> None:
        ...
        if not choices:
            ...
            if not choices:
                if silent:
                    ...
                    hass.states.async_set(
                        "sensor.shopping_list_with_grocy_polling_voice_response_helper",
                        "error",
                        {
                    ...
                    await hass.services.async_call(
                        ...
                        "create",
                        {
                            "title": no_choice_strings.get(
                                "title", "âŒ No Choices Available"
                            ),
        ...
        if choice_number < 1 or choice_number > len(matches):
            ...
            if not silent:
                ...
                title = invalid_choice_strings.get("title", "âŒ Invalid Choice")
        ...
        try:
            ...
            if not silent:
                ...
                title = choice_success_strings.get("title", "âœ… Product Selected")
        ...
        try:
            ...
            if todo_entity_id:
                if (
                    ...
                    and "shopping_list_with_grocy_polling" in todo_entity_id
                ...
                todo_entities = [
                    ...
                    and "shopping_list_with_grocy_polling" in entity_id
                ]
                ...
                if not silent:
                    ...
                    for i, match in enumerate(matches[:5], 1):
                        service_options.append(
                            f"{i}. {match['name']} â†’ product_id: {match['id']}"
                        )
