...
class GrocyShoppingSuggestions extends LitElement {
    ...
    _updateShoppingListState() {
        ...
        Object.entries(this.hass.states).forEach(([entityId, state]) => {
            if (entityId.startsWith('shopping_list_with_grocy_polling.')) {
    ...
    async _loadLocalTranslations() {
        ...
        const base = "/shopping_list_with_grocy_polling";
        const candidates = [
        ...
        for (const url of candidates) {
            ...
            this._translationsLoaded = true;
                this.hass?.loadBackendTranslation?.("shopping_list_with_grocy_polling");
            this.requestUpdate();
            return;
            } catch (e) {
                }
        }
        console.warn("âš ï¸ No local translations found, falling back to keys.");
    }
    ...
    async _refreshSuggestions() {
        ...
        try {
            this._loading = true;
            this.requestUpdate();
            await this.hass.callService('shopping_list_with_grocy_polling', 'suggest_grocery_list', {
    ...
    render() {
        ...
        if (this._loading) {
                emptyMessage = this.t('shopping_list_with_grocy_polling.ui.panel.analysis_progress');
        } else if (suggestions.length === 0) {
            if (lastUpdate) {
                        emptyMessage = this.t('shopping_list_with_grocy_polling.ui.panel.no_matches');
            } else {
                        emptyMessage = this.t('shopping_list_with_grocy_polling.ui.panel.no_analysis');
                showAddButton = false;
            }
        }
            ...
            <div class="view">
                <div class="header">
                    <div class="toolbar">
                        ...
                        <div class="main-title">${this.t('shopping_list_with_grocy_polling.ui.panel.title')}</div>
                        <ha-icon-button
                            @click=${this._refreshSuggestions}
                            .path=${"M17.65,6.35C16.2,4.9 14.21,4 12,4A8,8 0 0,0 4,12A8,8 0 0,0 12,20C15.73,20 18.84,17.45 19.73,14H17.65C16.83,16.33 14.61,18 12,18A6,6 0 0,1 6,12A6,6 0 0,1 12,6C13.66,6 15.14,6.69 16.22,7.78L13,11H20V4L17.65,6.35Z"}
                            label=${this.t('shopping_list_with_grocy_polling.ui.panel.refresh')}
                            ...
                            ? html`
                                <div class="card-actions">
                                    <mwc-button 
                                        ...
                                        ?disabled=${this._loading}>
                                        ${this.t('shopping_list_with_grocy_polling.ui.panel.add_all_count', { count: this._getSelectedCount(suggestions) })}
                                    </mwc-button>
                                </div>`
                            : ''}
                    </ha-card>
            ...
            <div class="suggestion-item">
                <div class="product-info">
                    ...
                    <div class="product-stats">
                        ${this.t('shopping_list_with_grocy_polling.ui.panel.stats.score', { score: suggestion.score.toFixed(2) })}
                    </div>
                </div>
                <div class="actions">
                    <div class="quantity-controls">
                        <ha-icon-button
                            ...
                            label=${this.t('shopping_list_with_grocy_polling.ui.panel.quantity.decrease')}
                            ?disabled=${quantity === 0}>
                        </ha-icon-button>
                        ...
                        <ha-icon-button
                            ...
                            label=${this.t('shopping_list_with_grocy_polling.ui.panel.quantity.increase')}>
                        </ha-icon-button>
                    </div>
                    <ha-icon-button
                        ...
                        label=${quantity > 0 
                            ? this.t('shopping_list_with_grocy_polling.ui.panel.quantity.add')
                            : this.t('shopping_list_with_grocy_polling.ui.panel.quantity.select')}
    ...
    async _addAllToList(suggestions) {
        ...
        try {
            ...
            for (const suggestion of productsToAdd) {
                const quantity = this.quantities[suggestion.id] || 0;
                await this.hass.callService('shopping_list_with_grocy_polling', 'add_product', {
