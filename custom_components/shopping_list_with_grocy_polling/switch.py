...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    switch = ShoppingListWithGrocySwitch(
        coordinator,
        "pause_update_shopping_list_with_grocy_polling",
        "ShoppingListWithGrocy Pause update",
    )

    hass.data[DOMAIN]["entities"]["pause_update_shopping_list_with_grocy_polling"] = switch
...
class ShoppingListWithGrocySwitch(SwitchEntity):
    def __init__(self, coordinator, object_id, name):
        unique_id = "pause_update_shopping_list_with_grocy_polling"
