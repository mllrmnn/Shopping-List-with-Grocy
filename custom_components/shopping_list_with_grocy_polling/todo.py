    ...
    async def async_create_todo_item(self, item: TodoItem) -> None:
        ...
        try:
            ...
            if result["success"]:
                ...
                if not voice_mode:
                    yaml_service = (
                        "service: shopping_list_with_grocy_polling.select_choice_by_number\n"
                        "data:\n"
                ...
                LOGGER.error(
                    "âŒ Failed to create item '%s': %s", item.summary, error_reason
                )
...
async def async_setup_entry(
    ...
    if not coordinator:
        LOGGER.error("âŒ No coordinator found for entry ID %s", config_entry.entry_id)
        return
