...
async def async_setup(hass: HomeAssistant, config: dict) -> bool:
    ...
    if DOMAIN not in hass.data:
        hass.data[DOMAIN] = {
            "_translation_key": "component.shopping_list_with_grocy_polling.ui.panel.title"
        }
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    if entry.version < 10:
        ...
        if not migration_success:
            LOGGER.error("âŒ Migration failed. Initialization stopped.")
            return False
    else:
        ...
        def _update_blueprints():
            ...
            for blueprint_name in blueprint_names:
                ...
                if not dest.exists() or file_sha256(src) != file_sha256(dest):
                    ...
                    LOGGER.info(
                        "ðŸ“„ Blueprint '%s' copied or updated to '%s'",
                        blueprint_name,
                        dest,
                    )
                    updated = True
...
async def remove_restored_entities(hass: HomeAssistant):
    ...
    rex = re.compile(
        r"^sensor\.shopping_list_with_grocy_polling_product_v1_.+|switch\.shoppinglistwithgrocy_pause_update|binary_sensor\.shoppinglistwithgrocy_update_in_progress$"
    )
